...
int FourSuite::testParseUri(const char * uriText, const char** errorPos) {
...
bool FourSuite::testBadUri(const char * uriText, const int errorOffset = 0) {
	const char* errorPos;
        int ret = testParseUri(uriText, &errorPos);
	return ((ret == URI_ERROR_SYNTAX) && (errorPos == (uriText + errorOffset)));
}
